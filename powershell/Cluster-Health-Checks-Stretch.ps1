...
Drive health
Check SR partnerships are healthy and in sync
...
Function Check-Resources($Resources){
    ...
    #Check SR Groups
    Write-Host "`nSR Groups:" -ForegroundColor Cyan
        $Resources = Get-SRGroup -CimSession $Cluster | sort Name
    Foreach($Resource in $Resources){
        If($Resource.ReplicationStatus -ne 'ContinuouslyReplicating'){
